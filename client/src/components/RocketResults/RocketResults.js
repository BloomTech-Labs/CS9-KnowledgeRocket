...
class RocketResult extends Component {
    ...
    generateChart = () => {
        ...
        for (let i = 0; i < choices.length; i++) {
            ...
            svgs.push(<path d={pathData} fill={colors[i % colors.length]} className='pieChartSection'/>);
        }

        return (
            // SVG VIEWBOX 1:2 ratio -1 -1 2 2 were originals
            <svg viewBox="-1.5 -1.5 3 3" style={{transform: 'rotate(-0.25turn)'}} space="preserve">
                {svgs.map(each => {
                    return each;
                })}
            </svg>
    ...
    render() {
        return (
            <div><div style={{width: '300px', height: '300px', padding: '1rem'}}>{this.generateChart()}</div>
